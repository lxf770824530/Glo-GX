...
from utils import Initial_graph_generate, Fix_nodes_index,load_checkpoint,Get_dataset_class_num, Draw_graph, Fix_gate
...
def Explain_model(args):
    ...
    while len(G.x) >= final_node_num:
        ...
        target_label = torch.full(size=(1, len(G.x)), fill_value=tar_class)  # arg+++++
        ...
        for epoch in range(1000):
            ...
            gnn_model.eval()
            pe = gnn_model(G.x, G.edge_index, G.edge_attr)
            ...
            gnn_model.eval()
            pred = gnn_model(G.x, G.edge_index, G.edge_attr)
            pred = torch.exp(pred, out=None)
            # pred, nodes_class = run_discriminator(gnn_model, G, args)
